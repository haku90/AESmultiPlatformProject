...
void InvMixColumns()
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		state[0][i] = x14[temp[0]] ^ x11[temp[1]] ^ x13[temp[2]] ^ x9[temp[3]];
		state[1][i] = x9[temp[0]] ^ x14[temp[1]] ^ x11[temp[2]] ^ x13[temp[3]];
		state[2][i] = x13[temp[0]] ^ x9[temp[1]] ^ x14[temp[2]] ^ x11[temp[3]];
		state[3][i] = x11[temp[0]] ^ x13[temp[1]] ^ x9[temp[2]] ^ x14[temp[3]];
	}
}
//Function Encrypt.
void Encrypt()
...
//Function DisplayOut.
void DisplayOut(unsigned char toDisp[16])
{
	int i;
	for (i = 0; i<Nb * 4; i++)
	{
...
void main()
{
	...
	Decrypt();
	TransferEncodeMessageToEncodeTable();
	printf("\nPo deszyfracji w hex:\n");
	DisplayOut(&out);

	unsigned char* fileNameWriteD = "DecodeMessage.txt";
	WriteFile(fileNameWriteD, &out);
	return 1;
}
